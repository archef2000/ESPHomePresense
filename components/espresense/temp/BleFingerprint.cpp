...
int bt_encrypt_be(const uint8_t *key, const uint8_t *plaintext, uint8_t *enc_data) {
    ...
    if (mbedtls_aes_setkey_enc(&ctx, key, 128) != 0) {
        mbedtls_aes_free(&cfingerprintManufactureDatatx);
...
void BleFingerprint::fingerprintServiceAdvertisements(NimBLEAdvertisedDevice *advertisedDevice, size_t serviceAdvCount, bool haveTxPower, int8_t txPower) {
...
#ifdef VERBOSE
        Serial.printf("Verbose | %s | %-58s%ddBm AD: %s\r\n", getMac().c_str(), getId().c_str(), rssi, advertisedDevice->getServiceUUID(i).tostd::string().c_str());
#endif
        if (uuid == tileUUID) {
    ...
    for (int i = 0; i < serviceAdvCount; i++) {
        std::string sid = advertisedDevice->getServiceUUID(i).tostd::string();
    ...
    for (int i = 0; i < serviceDataCount; i++) {
...
#ifdef VERBOSE
        Serial.printf("Verbose | %s | %-58s%ddBm SD: %s/%s\r\n", getMac().c_str(), getId().c_str(), rssi, uuid.tostd::string().c_str(), hexStr(strServiceData).c_str());
#endif
        ...
        if (uuid == exposureUUID) {  // found COVID-19 exposure tracker
            ...
            if (strServiceData[0] == EDDYSTONE_URL_FRAME_TYPE && strServiceData.length() <= 18) {
                ...
                mv = oBeacon.getVolt();
#ifdef VERBOSE
                Serial.println(oBeacon.tostd::string().c_str());
#endif
            ...
            fingerprint = fingerprint + uuid.tostd::string().c_str();
        }
    }
...
bool BleFingerprint::fill(JsonObject *doc) {
    ...
    (*doc)[F("int")] = (esphome::millis - firstSeenMillis) / seenCount;

    if (mv) (*doc)[F("mV")] = mv;
    if (battery != 0xFF) (*doc)[F("batt")] = battery;
    if (temp) (*doc)[F("temp")] = serialized(std::string(temp, 2));
    if (humidity) (*doc)[F("rh")] = serialized(std::string(humidity, 2));
    return true;
}
